...
// PushPowerOff powers off the system using Redfish.
func (r *RedfishBMC) PushPowerOff(systemUUID string) error {
	system, err := r.getSystemByUUID(systemUUID)
	if err != nil {
		return fmt.Errorf("failed to get systems: %w", err)
	}
	if err := system.Reset(redfish.PushPowerButtonResetType); err != nil {
		return fmt.Errorf("failed to reset system to power on state: %w", err)
	}
	return nil
}
